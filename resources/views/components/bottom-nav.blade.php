...
@php
    $tabs = [
        ...
        ['route' => 'expenses.create', 'match' => ['expenses.*'], 'label' => 'Utgift', 'icon' => 'plus'],
        ...
        <a href="{{ route($tab['route']) }}"
            ...
            @switch($tab['icon'])
                ...
                @case('building')
                    ...
                    @break
                @case('plus')
                    <span class="flex size-[23px] items-center justify-center rounded-full bg-terra">
                ...
                @case('inbox')
                    <span class="relative">
                        ...
                        @if ($count)
                            <span class="absolute -right-2 -top-1.5 inline-flex h-[16px] min-w-[16px] items-center justify-center rounded-full bg-terra px-1 text-[10px] font-bold leading-none text-white">{{ $count }}</span>
                        @endif
                    </span>
                    @break
                @case('trip')
